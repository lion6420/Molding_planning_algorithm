...
class preprocessing():
	...
	def get_color(self, plastic_number):
		data = self.api_oracle.queryFilterOne('plastic_color_data', plastic_part_NO=plastic_number)
		if data:
			return data[1]
		else:
			return 'others'
	...
	# 	data = self.api_oracle.queryFilterAll('MJ_DATA', HH_NO1=PN)
	# 	if data:
	...
	def get_planning_input(self):
		...
		for w_d in tqdm(self.weeklyDemand, ascii=True):
			...
			if amount>0:
				...
				name = self.api_oracle.queryFilterOne('MATERIAL', ITEM_NO=PN)[7]
